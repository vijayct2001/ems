...
class PetControllerTests {
	...
	@BeforeEach
	void setup() {
		PetType bell = new PetType();
		bell.setId(3);
		bell.setName("mariner");
		given(this.owners.findPetTypes()).willReturn(Lists.newArrayList(bell));
	...
	void testProcessCreationFormSuccess() throws Exception {
		mockMvc.perform(post("/owners/{ownerId}/pets/new", TEST_OWNER_ID).param("name", "Betty")
				.param("type", "mariner").param("birthDate", "2015-02-12")).andExpect(status().is3xxRedirection())
